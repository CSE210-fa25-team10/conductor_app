...
async function renderGroupInfo() {
    ...
    if (userRole === 'student') {
        ...
        const res = await fetch(`/api/groups/${courseId}`, {
            method: "GET",
            credentials: "include",
        });
        if (!res.ok) {
            const errorData = await res.json();
            console.error('Error fetching groups for course:', errorData);
            throw new Error(errorData.error || 'Failed to fetch groups for course');
        }
        const AllGroupsData = await res.json();
        const groupsArray = AllGroupsData.groups;
        ...
        if (!groupsArray || groupsArray.length === 0) {
            ...
            <div class="section">
                ...
                <div class="empty-state" style="padding: 40px; text-align: center; border: 1px dashed #ccc; border-radius: 8px;">
                    <p style="color: #666; font-size: 1.1em;">
                        👋 No groups have been created yet for this course.
                    </p>
                    <p style="color: #999; font-size: 0.9em;">
                        Once groups are formed, they will appear here.
                    </p>
                </div>
            </div>
        `;
