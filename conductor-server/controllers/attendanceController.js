...
//
export async function startAttendanceSession(req, res) {
...
// - list of activities
//
export async function getCourseAttendanceSummary(req, res) {
...
// }
//
export async function getCourseGroupAndMemberOverview(req, res) {
  const courseId = Number.parseInt(req.params.courseId, 10);
  if (!Number.isInteger(courseId)) {
    return res.status(400).json({ error: 'course_id must be an integer' });
  }
...
// }
//
export async function getStudentCourseAttendanceOverview(req, res) {
  const courseId = Number.parseInt(req.params.courseId, 10);
